...
def generate_new_daily_melt_files(
    start_date="1978-01-01", end_date=None, overwrite=True, warn_if_missing_files=True
):
...
def read_and_parse_args():
    ...
    parser.add_argument(
        "Tb_file_19h", type=str, help="A daily NSIDC Polar-stereo Tb files (.bin)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Output a GeoTiff (.tif) in addtion to the flat binary.",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
if __name__ == "__main__":
    generate_new_daily_melt_files(start_date="1978-01-01")
